...
function generateLocalIntelligencePalette(request) {
  try {
    ...
    var secondaryColor;
    try {
      var color = ColorJsIo.parseColor(request.baseColor);
      var oklchCoords = ColorJsIo.getOklchCoords(color);
      var h = oklchCoords[2];
      var l = oklchCoords[0];
      var c = oklchCoords[1];
      secondaryColor = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h + 30.0) % 360.0));
    }
    catch (exn){
    ...
    var neutralColor;
    try {
      var color$1 = ColorJsIo.parseColor(request.baseColor);
      var oklchCoords$1 = ColorJsIo.getOklchCoords(color$1);
      var h$1 = oklchCoords$1[2];
      neutralColor = ColorJsIo.toHex(ColorJsIo.oklch(0.5, 0.05, h$1));
    }
    catch (exn$1){
      neutralColor = "#6b7280";
    }
...
function generateRuleBasedPalette(request) {
  try {
    var baseColor = ColorJsIo.parseColor(request.baseColor);
    var baseCoords = ColorJsIo.getOklchCoords(baseColor);
    var h = baseCoords[2];
    var c = baseCoords[1];
    var generateSimpleScale = function (color) {
      var tokens = {};
      var steps = [
        50.0,
        100.0,
      ...
      Belt_Array.forEach(steps, (function (step) {
              ...
              var colorHex;
              try {
                colorHex = ColorJsIo.toHex(ColorJsIo.oklch(lightness, c, h));
              }
              catch (exn){
    ...
    var secondaryColor;
    try {
      secondaryColor = ColorJsIo.oklch(baseCoords[0], c, (h + 30.0) % 360.0);
    }
    catch (exn){
    ...
    var neutralColor;
    try {
      neutralColor = ColorJsIo.oklch(0.5, 0.05, h);
    }
    catch (exn$1){
      neutralColor = ColorJsIo.parseColor("#6b7280");
    }
    var tokens = {
      color: {
