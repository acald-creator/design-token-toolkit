...
import * as ColorJsIo from "./ColorJsIo.res.mjs";
...
function analyzeColorAI(baseColor) {
  try {
    var color = ColorJsIo.parseColor(baseColor);
    var oklchCoords = ColorJsIo.getOklchCoords(color);
    var l = oklchCoords[0];
    var c = oklchCoords[1];
    var h = oklchCoords[2];
...
function generateHarmonyPalette(baseColor, harmony) {
  try {
    ...
    if (harmony === "analogous") {
      var color1 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h - 30.0) % 360.0));
      var color3 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h + 30.0) % 360.0));
      var color4 = ColorJsIo.toHex(ColorJsIo.oklch(l * 1.1, c * 0.8, (h - 15.0) % 360.0));
      var color5 = ColorJsIo.toHex(ColorJsIo.oklch(l * 0.9, c * 0.8, (h + 15.0) % 360.0));
      return [
              color1,
              baseColor,
              color3,
    ...
    if (harmony === "triadic") {
      var triadic1 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h + 120.0) % 360.0));
      var triadic2 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h + 240.0) % 360.0));
      return [
              baseColor,
              triadic1,
              triadic2
    ...
    if (harmony === "monochromatic") {
      var lighter1 = ColorJsIo.toHex(ColorJsIo.oklch(Math.min(0.95, l + 0.2), c, h));
      var lighter2 = ColorJsIo.toHex(ColorJsIo.oklch(Math.min(0.9, l + 0.1), c, h));
      var darker1 = ColorJsIo.toHex(ColorJsIo.oklch(Math.max(0.1, l - 0.1), c, h));
      var darker2 = ColorJsIo.toHex(ColorJsIo.oklch(Math.max(0.05, l - 0.2), c, h));
      return [
              lighter1,
              lighter2,
              baseColor,
    ...
    var complementary = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h + 180.0) % 360.0));
    return [
            baseColor,
            complementary
          ];
...
function generateAccessibleCombination(backgroundColor, textColorOpt, param) {
  ...
  try {
    var bgColor = ColorJsIo.parseColor(backgroundColor);
    var bgCoords = ColorJsIo.getOklchCoords(bgColor);
...
function generateSemanticColorsAI(brandColors) {
  ...
  var primaryColor = ColorJsIo.parseColor(brandColors.primary);
  var primaryCoords = ColorJsIo.getOklchCoords(primaryColor);
  var neutral;
  try {
    var h = primaryCoords[2];
    neutral = ColorJsIo.toHex(ColorJsIo.oklch(0.5, 0.05, h));
  }
  catch (exn){
    neutral = "#6b7280";
  }
...
function suggestBrandColorsAI(baseColor) {
  ...
  try {
    ...
    var analogous1 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h + 30.0) % 360.0));
    suggestions.push({
          color: analogous1,
    ...
    var analogous2 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h - 30.0) % 360.0));
    suggestions.push({
          color: analogous2,
    ...
    if (currentAnalysis.properties.accessibility === "poor") {
      var improved = ColorJsIo.toHex(ColorJsIo.oklch(l > 0.5 ? 0.3 : 0.7, c, h));
      suggestions.push({
            color: improved,
  ...
  try {
    ...
    if (basePalette !== undefined) {
      ...
      if (saturationAdj !== 0.0 || lightnessAdj !== 0.0) {
        var adjusted100;
        try {
          adjusted100 = ColorJsIo.toHex(ColorJsIo.oklch(Math.max(0.05, Math.min(0.95, l + lightnessAdj)), Math.max(0.0, c + saturationAdj), h));
        }
        catch (exn){
...
function generateColorPaletteEnhanced(baseColor, accessibilityOpt, param) {
  try {
    ...
    var colors = Belt_Array.mapWithIndex(lightnessScale, (function (index, lightness) {
            try {
              return ColorJsIo.toHex(ColorJsIo.oklch(lightness, c, h));
            }
            catch (exn){
              return baseColor;
            }
...
function generateHarmoniousPaletteEnhanced(baseColor, harmonyOpt, param) {
  ...
  try {
    ...
    if (harmony === "analogous") {
      var color1 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h - 30.0) % 360.0));
      var color2 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, h));
      var color3 = ColorJsIo.toHex(ColorJsIo.oklch(l, c, (h + 30.0) % 360.0));
      var color4 = ColorJsIo.toHex(ColorJsIo.oklch(l * 0.8, c, (h + 15.0) % 360.0));
      var color5 = ColorJsIo.toHex(ColorJsIo.oklch(l * 1.2, c, (h - 15.0) % 360.0));
      return [
              color1,
              color2,
              color3,
    ...
    if (harmony === "monochromatic") {
      var lighter1 = ColorJsIo.toHex(ColorJsIo.oklch(Math.min(0.95, l + 0.2), c * 0.8, h));
      var lighter2 = ColorJsIo.toHex(ColorJsIo.oklch(Math.min(0.95, l + 0.1), c * 0.9, h));
      var darker1 = ColorJsIo.toHex(ColorJsIo.oklch(Math.max(0.05, l - 0.1), c * 0.9, h));
      var darker2 = ColorJsIo.toHex(ColorJsIo.oklch(Math.max(0.05, l - 0.2), c * 0.8, h));
      return [
              lighter1,
              lighter2,
              baseColor,
